...
var current_state setget _set_current_state
...
func _set_current_state(state: String) -> void:
	if state and not _states_stack.front() == state:
		...
		if current_state and current_state.is_connected('state_changed', self, '_on_State_state_changed'):
			current_state.disconnect('state_changed', self, '_on_State_state_changed')
...
# Signal bind
func _on_State_state_changed(state: String) -> void:
	go_to(state)
